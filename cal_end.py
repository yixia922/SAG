...
def cal_indices_end(data1, data2, data3, indices):
    if indices == 'TNn':
        ...
        result = cal_CDD_end(data1, data2, data3)
    elif indices == 'CWD':
        result = cal_CWD_end(data1, data2, data3)
    elif indices == 'R10mm':
...
# calculate maximum duration of dry days (daily precipitation <= 1mm) in a given year
def max_cons_CDD(xr_obj):
    overall_counter = []; count = 0
    for i in range(len(xr_obj)):
        count = count + 1 if xr_obj[i] <= 1 else 0
        overall_counter.append(count)
    return max(overall_counter)


def cal_CDD_end(data1, data2, data3):
    # calculate Consecutive Dry Day (CDD) in each year
    len_year = 21
    len_lat = len(data1.lat)
    len_lon = len(data1.lon)
    CDD = np.zeros((len_year, len_lat, len_lon))  # To store the CDD results

    # For p1 (year 2075-2079)
    for year in range(2075, 2079 + 1):
        CDD_now = data1.PRECT.sel(time='%d'%year)
        CDD_now = CDD_now * 1000 * 3600 * 24   # change unit from m/s to mm/day
        for i in range(192):  # loop for lat
            for j in range(288):  # loop for lon
                CDD[year - 2075, i, j] = max_cons_CDD(CDD_now[:, i, j])
    # For p2 (year 2080-2089)
    for year in range(2080, 2089 + 1):
        CDD_now = data2.PRECT.sel(time='%d'%year)
        CDD_now = CDD_now * 1000 * 3600 * 24   # change unit from m/s to mm/day
        for i in range(192):  # loop for lat
            for j in range(288):  # loop for lon
                CDD[year - 2075, i, j] = max_cons(CDD_now[:, i, j])
    # For p3 (year 2090-2095)
    for year in range(2090, 2095 + 1):
        CDD_now = data3.PRECT.sel(time='%d'%year)
        CDD_now = CDD_now * 1000 * 3600 * 24   # change unit from m/s to mm/day
        for i in range(192):  # loop for lat
            for j in range(288):  # loop for lon
                CDD[year - 2075, i, j] = max_cons(CDD_now[:, i, j])
